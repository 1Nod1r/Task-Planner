...
class MyProfileView: BaseView {
    ...
    private func createTopViews(){
        ...
        addSubview(image)
        let nameLabel = Label(title: "Denko Neva", txtColor: AppTheme.current.textTitleColor, txtFont: UIFont(name: Almarai.almaraiBold, size: 20), txtLines: 1, txtAlignment: .center)
        self.nameLabel = nameLabel
        self.addSubview(nameLabel)
        let button = UIButton.highLightedButton(title: "Log out", textColor: AppTheme.current.buttonTint, size: 14)
        self.logOut = button
        ...
        table.borderColor = AppTheme.current.profileBorderColor
        table.borderWidth = 1
