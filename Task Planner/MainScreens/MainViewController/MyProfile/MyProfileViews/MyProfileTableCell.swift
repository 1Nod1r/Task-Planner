...
class MyProfileTableCell: BaseTableCell, ReusableView {
    ...
    private func createViews(){
        ...
        let lineView = UIView()
        lineView.backgroundColor = .AppColors.xC4D7FF
        self.lineView = lineView
        addSubview(lineView)
        let image = DefaultImageView(imageName: "")
        self.image = image
        let subtitle = Label(txtColor: .AppColors.x585A66, txtFont: UIFont(name: Almarai.almaraiRegular, size: 12), txtLines: 1)
        self.subtitle = subtitle
        let title = Label(txtColor: .AppColors.x585A66, txtFont: UIFont(name: Almarai.almaraiRegular, size: 16), txtLines: 1)
        self.title = title
